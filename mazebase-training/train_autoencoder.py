...
#augment...
    ...
    #print(total)
    #limit = int(0.05*total)
    #print(limit)

    #keep_list = random.sample(onefouractions, limit) + random.sample(fiveactions, limit)
    percent = 0.05
...
def train_step_autoencoder(epoch):
    log_size = 500
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        state_encoding = model.get_state_encoding(states_embedding, states_onehot, inventory, goal, instructions, lengths, device)

        outputs = model2(state_encoding)
        
        recon_loss = criterion2(outputs, state_encoding)
        recon_loss.backward()
